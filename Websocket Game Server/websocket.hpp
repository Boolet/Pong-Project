...
typedef void (*messageCallback)(int, std::string);
...
public:
    ...
    std::string MessageBuffer;             // a blank string when there's no incoming frames
    ...
    std::string FrameBuffer;               // joined onto end as a frame's data comes in, reset to blank string when all frame data has been read
...
public:
    ...
    void stopServer();
    bool wsSend(int clientID, std::string message, bool binary = false);
...
private:
    ...
    bool wsSendClientMessage(int clientID, unsigned char opcode, std::string message);
    ...
    bool wsProcessClientMessage(int clientID, unsigned char opcode, std::string data, int dataLength);
